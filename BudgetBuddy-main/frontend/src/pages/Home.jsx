...
const Home = () => {
  ...
  const capitalizeFirstLetter = (string) => typeof string === 'string' ? string.charAt(0).toUpperCase() + string.slice(1).toLowerCase() : '';

  const filteredInvestments = useMemo(() => investments?.filter(i => capitalizeFirstLetter(new Date(i.createdAt).toLocaleString('default', { month: 'long' })) === capitalizeFirstLetter(selectedMonth)) || [], [investments, selectedMonth]);
  const totalInvestmentValue = useMemo(() => filteredInvestments.reduce((sum, i) => sum + i.amount, 0), [filteredInvestments]);

  const filteredBudgets = useMemo(() => budgets?.filter(b => capitalizeFirstLetter(new Date(b.createdAt).toLocaleString('default', { month: 'long' })) === capitalizeFirstLetter(selectedMonth)) || [], [budgets, selectedMonth]);
  const totalBudgetValue = useMemo(() => filteredBudgets.reduce((sum, b) => sum + b.amount, 0), [filteredBudgets]);

  const filteredFiles = useMemo(() => files?.filter(f => capitalizeFirstLetter(new Date(f.date).toLocaleString('default', { month: 'long' })) === capitalizeFirstLetter(selectedMonthStatements)) || [], [files, selectedMonthStatements]);
  const totalStatementValue = useMemo(() => filteredFiles.reduce((sum, f) => sum - f.amount, 0), [filteredFiles]);

  const filteredIncomes = useMemo(() => incomes?.filter(i => capitalizeFirstLetter(new Date(i.createdAt).toLocaleString('default', { month: 'long' })) === capitalizeFirstLetter(selectedMonthStatements)) || [], [incomes, selectedMonthStatements]);
  const totalIncomeValue = useMemo(() => filteredIncomes.reduce((sum, i) => sum + i.amount, 0), [filteredIncomes]);
  ...
  const renderView = () => {
    switch (activeView) {
      case "investments": return <div className="home"><ToggleInvestmentForm setActiveViewInvestment={setActiveViewInvestment} /></div>;
      case "budgets": return (
        <div className="home">
          <div className="budgets">
            ...
            <form className="create">
              <label>Month:</label>
              <select value={selectedMonth} onChange={handleMonthChange}>{months.map(month => <option key={month} value={month}>{month}</option>)}</select>
            </form>
            <BudgetDiffChart selectedMonth={selectedMonth} />
            {filteredBudgets.length > 0 ? filteredBudgets.map(b => <BudgetDetails key={b._id} budget={b} />) : <p>No budgets for the selected month.</p>}
          </div>
          <BudgetForm />
        </div>
      );
      case "incomes": return (
        <div className="home">
          <div className="incomes">
            ...
            <form className="create">
              <label>Month:</label>
              <select value={selectedMonthStatements} onChange={handleMonthChangeStatements}>{months.map(month => <option key={month} value={month}>{month}</option>)}</select>
            </form>
            <IncomePieChart selectedMonth={selectedMonthStatements} />
            {filteredIncomes.length > 0 ? filteredIncomes.map(i => <IncomeDetails key={i._id} income={i} />) : <p>No incomes for the selected month.</p>}
          </div>
          <IncomeForm />
        </div>
      );
      case "statements": return (
        <div className="home">
          <div className="budgets">
            ...
            {filteredFiles.length > 0 ? filteredFiles.map(f => <StatementDetails key={f._id} file={f} />) : <p>No statements for the selected month.</p>}
          </div>
          <StatementUpload />
        </div>
      );
      case "spendingSummary": return <div className="home"><h2>Spending Summary</h2><SpendingSummary /></div>;
      case "notifications": return <div className="home"><div className="investments">{notifications?.map(n => <NotificationDetails key={n._id} notification={n} />)}</div></div>;
      default: return <div className="home"></div>;
    }
  };

  const renderViewInvestment = () => {
    switch (activeViewInvestment) {
      case "investmentAnalysis": return (
        <div className="home">
          <div className="investments">
            ...
            {filteredInvestments.length > 0 ? filteredInvestments.map(i => <InvestmentDetails key={i._id} investment={i} />) : <p>No investments for the selected month.</p>}
            <h3>Total Investment Value: ${totalInvestmentValue.toFixed(2)}</h3>
          </div>
        </div>
      );
      case "investmentForm": return <div className="home"><div className="investments"><InvestmentForm /></div></div>;
      default: return <div className="home"></div>;
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      for (const { url, dispatch, type } of endpoints) {
        const res = await fetch(`${import.meta.env.VITE_API_URL}${url}`, { headers: { 'Authorization': `Bearer ${user.token}` } });
